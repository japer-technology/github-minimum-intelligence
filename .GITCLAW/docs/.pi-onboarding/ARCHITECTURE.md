...
```
GitHub Issue → GitHub Actions → lifecycle/main.ts → pi agent → .pi config
                                      │                          │
                                      │                          ├── reads settings.json
...
3. `lifecycle/main.ts` resolves the session (new or resumed), builds a prompt, and launches the `pi` agent binary.
...
6. After the agent responds, `main.ts` commits the session log, pushes changes, and posts the reply as an issue comment.
...
1. `main.ts` checks `state/issues/{number}.json` — not found.
...
1. `main.ts` finds the mapping in `state/issues/{number}.json`.
...
GitHub Actions triggers workflow
        │
        ▼
preinstall.ts adds 👀 reaction
        │
        ▼
main.ts resolves session
        │
        ▼
...
Session appended to state/sessions/*.jsonl
        │
        ▼
main.ts commits all changes to git
        │
        ▼
main.ts posts reply as issue comment
        │
        ▼
👀 reaction removed
```
